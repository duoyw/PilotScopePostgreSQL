...
print_path(PlannerInfo *root, Path *path, int indent)
{
	...
	switch (jointype)
	{
		...
		case JOIN_UNIQUE_OUTER:
			printf(" unique outer join \n");
			break;
		case JOIN_UNIQUE_INNER:
			printf(" unique inner join \n");
			break;
		default:
			printf(" unkown join type: %d \n", (int) jointype);
			break;
	}
		for (i = 0; i < indent; i++)
			printf("\t");
		...
		if (jp->joinrestrictinfo){
			...
			printf("@@@@@ ");
			print_restrictclauses(root, jp->innerjoinpath->param_info->ppi_clauses);
		}
		printf("\n");
